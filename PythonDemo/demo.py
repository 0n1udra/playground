...
WIDTH, message, printedMessage = 79, "hello!".upper(), [ "","","","","","","" ]
...
#the second line, etc..
for row in range(7):
    for char in message: printedMessage[row] += (str(characters[char][row]) + "  ")
...
while True:
    ...
    for row in range(7): print(" " * offset + printedMessage[row][max(0,offset*-1):WIDTH - offset])
    ...
    if offset <= ((len(message)+2)*6) * -1: offset = WIDTH
