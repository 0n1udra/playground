...
score, timeleft = 0, 30 #the player's score, initially 0. the game time left, initially 30 seconds.
...
def startGame(event): #a function that will start the game.
    if timeleft == timeSet: countdown() #if there's still time left...
...
def nextColour(): #function to choose and display the next colour.
    ...
    if timeleft > 0:
        ...
        if e.get().lower() == colours[1].lower(): score += 1 #if the colour typed is equal to the colour of the text...
...
font = 'Verdana', 14 # makes it easy to chagne most of the fonts
instructions = tkinter.Label(root, text="Type in the colour of the words, and not the word text!", font=(font)) #add an instructions label.
instructions.pack() #add a score label.
scoreLabel = tkinter.Label(root, text="Press enter to start", font=(font))
scoreLabel.pack() #add a time left label.
timeLabel = tkinter.Label(root, text="Time left: " + str(timeleft), font=(font))
timeLabel.pack() #add a label for displaying the colours.
...
e.pack(), e.focus_set(), root.mainloop() # 1. 2 set focus on the entry box. 3 start the GUI.
